...
void draw_line_x(double x1, double y1, double x2, double y2) {  // only works if x2 > x1
   ...
   for (x = xstart; x < xstop; ++x) {
      iy = floor(y);
      set_pixel_alpha(x, iy, line_color[0], line_color[1], line_color[2], line_color[3]);
      y += m;
      }
   }
...
void draw_line_y(double x1, double y1, double x2, double y2) {
 ...
 int main(int argc, char** argv) {  
   ...
   for (i = 0; i <= n; ++i) {
	...
	write_bmp((char*) "TransparentOrange_Line_x.bmp", (char*) image, image_size, width);
	return 1;
}
